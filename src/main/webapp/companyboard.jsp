...
<link rel="stylesheet" href="companyboardCss.css">
</head>
<body>
	<header>
		<div class="logo">
	...
	<table>
		<tr>
			<td>번호</td><td>제목</td><td>모집인원</td><td>업체명</td><td>업체위치</td><td>작성일</td>
		</tr>
		<c:forEach var="dto" items="${list}">
			<tr>
				<td id="copNo">${dto.no}</td>
				<td id="copOverflow">
					<a href="companyBoardView.do?no=${dto.no}&companyname=${dto.companyname}">
					${dto.title}
					</a>
				</td>
				<td id="copPeoplecount">${dto.peoplecount}</td>
				<td id="copName">${dto.companyname}</td>
				<td id="copRoadAddress">${dto.roadAddress}</td>
				<td id="copWtime">${dto.wtime}</td>
			</tr>
		</c:forEach>
	</table>
	<div class="writeText">
	<c:if test="${companyInfo.id ne null}">
	<div id="copwrite">
		<a href="companyBoardWrite.do" style="color:white">글쓰기</a>
	</div>
	</c:if>
	</div>
	<div class="page">
	...
	<c:forEach var="i" begin="0" end="${totalPage}" step="1">
		<c:choose>
			...
			<c:when test="${i ne curPage}">
				<a href="companyboard.do?curPage=${i}" style="color:white">${i+1}</a>
			</c:when>
		</c:choose>
	</c:forEach>
	</div>
