...
<link rel="stylesheet" href="userBoardViewCss.css">
</head>
<body>
	<header>
		<div class="logo">
	...
	<section>
	<form action="userBoardModifyOK.do" method="post">
	<div class="previewmenu">
		...
		<table>
			...
			<tr>
				<td id="programcount">활용프로그램 수<input type="text" name="programcount" id="pcInput" <c:if test="${userInfo.id eq null or userInfo.id ne dto.id}">readonly</c:if> value="${dto.programcount}"></td>
			</tr>
			<tr>
				<td>
					<hr align="center"/>
			...
			<tr>
				<td>
					<textarea name="contents" rows="35" cols="76" <c:if test="${userInfo.id eq null or userInfo.id ne dto.id}">readonly</c:if>>${dto.contents}</textarea>
					<hr align="center"/>
				</td>
			</tr>
		</table>
		...
		<a href="userboard.do"><button type="button">글목록</button></a>
		<c:if test="${userInfo.id ne null and userInfo.id eq dto.id or adminInfo ne null}">
			<c:if test="${adminInfo eq null}">
				<input type="submit" value="수정">
				<a href="userBoardDelete.do?no=${dto.no}"><button type="button">삭제</button></a>
			</c:if>
			<c:if test="${adminInfo ne null}">
				<a href="userBoardDelete.do?no=${dto.no}"><button type="button" style="width:140px;background-color:red">관리자 권한으로 삭제</button></a>
			</c:if>
		</c:if>
