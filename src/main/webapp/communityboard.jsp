...
<link rel="stylesheet" href="communityboardCss.css">
</head>
<body>
	<header>
		<div class="logo">
	...
	<div class="previewmenu">
		...
		<table>
			...
			<tr>
				<td id="commuNo">${dto.no}</td>
				<td id="commuOverflow">
					<a href="communityBoardView.do?no=${dto.no}&nickname=${dto.nickname}">
					<c:forEach var="i" begin="1" end="${dto.emptynum}">-</c:forEach>
					${dto.title}
					</a>
				</td>
				<td id="commuNickname">${dto.nickname}</td>
				<td id="commuWtime">${dto.wtime}</td>
				<td id="commuViewcount">${dto.viewcount}</td>
			</tr>
			</c:forEach>	
		</table>
	<div class="writeText">
	...
	<div id="commuwrite">
		<a href="communiyBoardWrite.do" style="color:white">글쓰기</a>
	</div>
	</c:if>
	</div>
	<div class="page">
	...
	<c:forEach var="i" begin="0" end="${totalPage}" step="1">
		<c:choose>
			...
			<c:when test="${i ne curPage}">
				<a href="communityboard.do?curPage=${i}" style="color:white">${i+1}</a>
			</c:when>
		</c:choose>
	</c:forEach>
	</div>
